...
Below is a sample deployment manifest known to work with cf-146.
 Substitute the new IP address you created in the last step for the
address 107.20.148.206, below. Obtain your director_uuid by executing
"bosh status" and updating the manifest below. No other changes are
required unless you are using a different version than cf-146.

This sample deployment was originally created via the bosh-cloudfoundry gem, located [here](https://github.com/cloudfoundry-community/bosh-cloudfoundry/blob/master/templates/v146/aws/medium/deployment_file.yml.erb).
...
created and deploy CF from the BOSH server to AWS. Let’s now do this.
